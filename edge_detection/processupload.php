...
<script> 
        ...
        $('#toggleDesaturate').click(function(){
            ...
            if($.browser.msie){
                edgeDetectionIE(imgObj);
            } else {
                imgObj.src = edgeDetection(imgObj);
            }           
        });
    });
      ...
      function edgeDetection(imgObj)
    {
var kkj=document.getElementById("kkk");
var direction=kkj.value;	   
		...
		for(var y = 1; y < imgPixels.height-1; y++){
            for(var x = 1; x < imgPixels.width-1; x++){
                ...
                var avg1= (imgPixels.data[i-((imgPixels.width-1)*4)-4]*a[0] + imgPixels.data[i -(imgPixels.width-1)*4]*a[1] + imgPixels.data[i -((imgPixels.width-1)*4)+4]*a[2] +imgPixels.data[i -4]*a[3] +imgPixels.data[i ]*a[4]+imgPixels.data[i +4]*a[5]+imgPixels.data[i +((imgPixels.width-1)*4)-4]*a[6]+imgPixels.data[i +((imgPixels.width-1)*4)]*a[7]+imgPixels.data[i +((imgPixels.width-1)*4)+4]*a[8]);
                var avg2= (imgPixels.data[i-((imgPixels.width-1)*4)-4+1]*a[0] + imgPixels.data[i -((imgPixels.width-1)*4)+1]*a[1] + imgPixels.data[i -((imgPixels.width-1)*4)+4+1]*a[2] +imgPixels.data[i -4+1]*a[3] +imgPixels.data[i +1]*a[4]+imgPixels.data[i +4+1]*a[5]+imgPixels.data[i +((imgPixels.width-1)*4)-4+1]*a[6]+imgPixels.data[i +((imgPixels.width-1)*4)+1]*a[7]+imgPixels.data[i +((imgPixels.width-1)*4)+4+1]*a[8]);
				var avg3= (imgPixels.data[i-((imgPixels.width-1)*4)-4+2]*a[0] + imgPixels.data[i -((imgPixels.width-1)*4)+2]*a[1] + imgPixels.data[i -((imgPixels.width-1)*4)+4+2]*a[2] +imgPixels.data[i -4+2]*a[3] +imgPixels.data[i+2 ]*a[4]+imgPixels.data[i +4+2]*a[5]+imgPixels.data[i +((imgPixels.width-1)*4)-4+2]*a[6]+imgPixels.data[i +((imgPixels.width-1)*4)+2]*a[7]+imgPixels.data[i +((imgPixels.width-1)*4)+4+2]*a[8]);
                src.data[i] = (avg1); 
				src.data[i + 1] = (avg2); 
                src.data[i + 2] = (avg3);
            }
