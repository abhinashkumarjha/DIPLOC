...
<script> 
       $(document).ready(function(){
        $('#toggleDesaturate').click(function(){
            ...
            if($.browser.msie){
                contrastImageIE(imgObj);
            } else {
                imgObj.src = contrastImage(imgObj);
            }           
        });
    });
...
var amount=kkj.value;
        ...
        for(var y = 0; y < imgPixels.height; y++){
            for(var x = 0; x < imgPixels.width; x++){
                ...
                if(imgPixels.data[i]>0 && imgPixels.data[i]<=80)
				{
                     imgPixels.data[i]=imgPixels.data[i]-(0.4*amount);
                }
                else if(imgPixels.data[i]>80 && imgPixels.data[i]<=140)
                {
        			 imgPixels.data[i]=imgPixels.data[i]-(0.4*amount)+2;
				}
                else if(imgPixels.data[i]>140 && imgPixels.data[i]<=210)
                {
        			 imgPixels.data[i]=imgPixels.data[i]+(0.4*amount)+2;
				}
				else if(imgPixels.data[i]>210)
                {
        			 imgPixels.data[i]=imgPixels.data[i]+(0.4*amount);
				}
				
				// Green channel pixels
				if(imgPixels.data[i+1]>=0 && imgPixels.data[i+1]<=80)
				{
                     imgPixels.data[i+1]=imgPixels.data[i+1]-(0.4*amount);
                }
                else if(imgPixels.data[i+1]>80 && imgPixels.data[i+1]<=140)
                {
        			 imgPixels.data[i+1]=imgPixels.data[i+1]-(0.4*amount)+2;
				}
                else if(imgPixels.data[i+1]>140 && imgPixels.data[i+1]<=210)
                {
        			 imgPixels.data[i+1]=imgPixels.data[i+1]+(0.4*amount)+2;
				}
				else if(imgPixels.data[i+1]>210)
                {
        			 imgPixels.data[i+1]=imgPixels.data[i+1]+(0.4*amount);
				}
				
				// Blue channel pixels
				if(imgPixels.data[i+2]>=0 && imgPixels.data[i+2]<=80)
				{
                     imgPixels.data[i+2]=imgPixels.data[i+2]-(0.4*amount);
                }
                else if(imgPixels.data[i+2]>80 && imgPixels.data[i+2]<=140)
                {
        			 imgPixels.data[i+2]=imgPixels.data[i+2]-(0.4*amount)+2;
				}
                else if(imgPixels.data[i+2]>140 && imgPixels.data[i+2]<=210)
                {
        			 imgPixels.data[i+2]=imgPixels.data[i+2]+(0.4*amount)+2;
				}
				else if(imgPixels.data[i+2]>210)
                {
        			 imgPixels.data[i+2]=imgPixels.data[i+2]+(0.4*amount);
				}
				
				// checking limits
				if(imgPixels.data[i]>255)
				{
                     imgPixels.data[i]=255;
                }
                else if(imgPixels.data[i]<0)
				{
                     imgPixels.data[i]=0;
                }
				
				if(imgPixels.data[i+1]>255)
				{
                     imgPixels.data[i+1]=255;
                }
                else if(imgPixels.data[i+1]<0)
				{
                     imgPixels.data[i+1]=0;
                }
                
				if(imgPixels.data[i+2]>255)
				{
                     imgPixels.data[i+2]=255;
                }
                else if(imgPixels.data[i+2]<0)
				{
                     imgPixels.data[i]=0;
                }                
            }
